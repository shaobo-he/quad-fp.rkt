...
(provide
 ...
 ;; unary math
 qfabs qfsqrt qfcbrt
 qfsin qfcos qftan qfasin qfacos qfatan
 qfsinh qfcosh qftanh qfasinh qfacosh qfatanh
 qfexp qfexp2 qfexpm1 qflog qflog2 qflog10 qflog1p qflogb
 qfceil qffloor qftrunc qfround qfrint qfnearbyint
 qferf qferfc qflgamma qftgamma
 qfj0 qfj1 qfy0 qfy1
 ;; binary / ternary math
 qfpow qfatan2 qfhypot qffmod qfremainder qfcopysign
 qffdim qfmax qfmin qfnextafter qffma
 ;; relations
 qf= qf< qf<= qf> qf>=
 ;; classification
 qfnan? qfinfinite? qffinite? qfsignbit? qfsignaling?
 ;; conversions
 double-flonum->quad-flonum quad-flonum->double-flonum
 string->quad-flonum quad-flonum->string
 ;; struct
 Quad? Quad-fh Quad-sh
 ;; constants
 quad-pi quad-pi/2 quad-pi/4 quad-1/pi quad-2/pi quad-2/sqrt-pi
 quad-e quad-log2e quad-log10e quad-ln2 quad-ln10
 quad-sqrt2 quad-1/sqrt2
 quad-max quad-min quad-epsilon quad-denorm-min
 quad-mant-dig quad-decimal-dig
 quad-min-exp quad-max-exp quad-min-10-exp quad-max-10-exp)
...
(define-syntax-rule (define-quad-predicate name #:c-id c-id)
  (begin
    ...
    (define (name a) (not (zero? (c-pred a))))))
...
(define-quad string->quad-flonum
  (_fun _string (r : (_ptr o _Quad)) -> _void -> r)
  #:c-id str2qf)

(define-quad quad-flonum->string/raw
  (_fun _Quad-pointer _bytes _int _int -> _int)
  #:c-id qf2str)
